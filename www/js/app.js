...
//if PhoneGap is not ready yet, wait for ondeviceready, else manually trigger ondeviceready on Chrome browser  only
if (EC.Utils.isChrome()) {
    console.log('deviceready Chrome');
    onDeviceReady();
}

//attach global handler for iOS, to be used when opening app using custom scheme epicollect5://key=value
window.localStorage.ios_project_form_url = "";
window.handleOpenURL = function(url) {"use strict";
...
function onDeviceReady() {"use strict";
    ...
    if (!EC.Utils.isChrome()) {
        ...
        switch(window.device.platform) {
            ...
            //on Android, it is possible to uniquely track a device (at the time of writing, KitKat 4.4.2 is the latest release)
 ...
 */
function onBackButton() {"use strict";

    var page_id = $.mobile.activePage.attr("id");

    //if the current page is the home page and the user press the back button, exit app
    if (page_id === "index") {
